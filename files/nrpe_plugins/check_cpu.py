...
#Usage message
usage = """usage: ./check_cpu.py [-w num|--warn=num] [-c|--crit=num] [-W num |--io-warn=num] [-C num|--io-crit=num] [-p num|--period=num] [-n num | --always-ok num]
    ...
    -n, --always-ok ... always return OK (0) status
...
def command_line_validate(argv):
  ...
  global always_ok
  try:
    opts, args = getopt.getopt(argv, 'w:c:o:W:C:i:I:s:n:S:p:f:VaA', ['warn=' ,'crit=', 'warn-any=', 'crit-any=', 'io-warn=','io-crit=','io-warn-overall=','io-crit-overall=','steal-warn=','always-ok=','steal-crit=','period=','version','--abs'])
  ...
  try:
    for opt, arg in opts:
      ...
      elif opt in ('-n', '--always-ok'):
        try:
          always_ok = int(arg)
        except:
          print '***always OK value must be 0 or 1***'
      elif opt in ('-a','--abs'):
        perfdata_abs = 3
